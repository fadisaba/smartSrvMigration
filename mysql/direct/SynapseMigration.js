...
/*
    ...
    migrateUser: async  function()
    {
        ...
        let usersArray =await dbUtilitySynapse.read({limit:"no",filters:filtersArray}, "user");
           ...
           /* if(userObj.id_user_profil===4)// medecin
            {
                ...
                let userPsArray =await dbUtilitySynapse.read({limit:"no",filters:[{name:'id_user',value:userObj.id_user,compare:'eq'}]}, "user_ps");
                if(userPsArray.length)
                {
                    for (let j = 0; j < userPsArray.length; j++) {
                        let psArray =await dbUtilitySynapse.read({limit:"no",filters:[{name:'id_ps',value:userPsArray[j].id_ps,compare:'eq'}]}, "ps");
                        if(psArray.length)
                        {
                            let psObj = psArray[0];
                            let userModel={};
    ...
    migratePS: async function () {
        ...
        let filtersArray = [{name: "obsolete", value: '0'}];
        let psArray = await dbUtilitySynapse.read({limit: "no", filters: filtersArray}, "ps");
        for (let i = 0; i < psArray.length; i++) {
           ...
           console.log(psObj.num_nat);
            let psSituationArray = await dbUtilitySynapse.read({
                limit: "no",
    ...
    migrateUnCorrespondant:async function(_obj,_rowsSpecialite)
    {
        ...
        if(row.id_coordonnee)
            rowsCoord= await dbUtilitySynapse.read({limit:"no",
        ...
        if(rowsCoord.length)
        {
            ...
            if(rowCoor.id_commune)
                rowsCommune= await dbUtilitySynapse.read({limit:"no",filters:
    ...
    migrateCorrespondant:async  function () {
        ...
        let rowsSpecialite =await dbUtilitySynapse.read({limit:"no",filters:[]}, "specialite");

       let rows =await dbUtilitySynapse.read({limit:"no",filters:[]}, "correspondant");
    ...
    migrateUnPatient:async function(_patientObj)
    {
        ...
        if(row.PATIENTCOMMUNEID)
            rowsCity= await dbUtilitySynapse.read({limit:"no",
                filters:[{name:'PATIENTCOMMUNEID',value:row.PATIENTCOMMUNEID,compare:'eq'}]}, "communes");


        let rowsCorresp= await dbUtility.read({limit:"no",filters:
                [{name:'referringPhysicianMigrationId',value:row.PRESCRIPTEURID+"",compare:'eq'}]}, "referring_physician");
       ...
       let  rowsPatientAdm= await dbUtilitySynapse.read({limit:"no",filters:
                [{name:'id_identite',value:row.id_identite,compare:'eq'}]}, "ident_adm");

       if(rowsPatientAdm.length)
       {
           let  rowsIdentPat= await dbUtilitySynapse.read({limit:"no",filters:
    ...
    migrateStudiesActe: function () {
        let dataToCreate=[];
        let filtersArray=[];
        dbUtilitySynapse.read({limit:"no",filters:filtersArray},'examen_cotation','')
            .then(function (rows) {
                rows.forEach(row=>{
                    let siteGroupeModel={};
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            if(_patientResultsArray && _patientResultsArray.length)
            {
              ...
              //  patientModel.visitMigrationField2=rowDossier.idCorrespondant2;
                ...
                let  rowsDossierAffichage= await  dbUtilitySynapse.read({limit:'no',
                ...
                if(rowDossier.id_ps)
                {
                    let  rowsPs= await  dbUtilitySynapse.read({limit:'no',
...
SynapseMigration.migratePatient(10,1);
