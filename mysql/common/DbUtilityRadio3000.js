...
let modelsSir5  = require('../modelsRadio3000');//uncomment for migration
"use strict";

let DbUtilityRadio3000 = {
    ...
    saveData:function(dataToBeAdded,dataToBeUpdated,dataToBeDeleted,table,idName,_transacVar)
    {
       // console.error(dataToBeUpdated);
        ...
        if(dataToBeAdded.length>0){
            let p1=DbUtilityRadio3000.insertRecords(dataToBeAdded,table,transacVar);
            promiseArray.push(p1);
        }
        if(dataToBeUpdated.length>0){
            let p2=DbUtilityRadio3000.updateRecords(dataToBeUpdated,table,idName,transacVar);
            promiseArray.push(p2);
        }
        if(dataToBeDeleted.length>0){
            let p3=DbUtilityRadio3000.deleteRecordsByIds(dataToBeDeleted,table,idName,transacVar);
    ...
    read: function(_params,_table,_dataBaseName="",_transac){
       ...
       return new Promise(
            function(resolve) {
                ...
                let model=DbUtilityRadio3000.getModelNameFormTableName(_table);
                let where=DbUtilityRadio3000.getFilters(_params.filters);
                let findAll={};
                findAll.where=where;
        ...
        return new Promise(
            function(resolve, reject) {
               ...
               if(_mainTableObject.filters)
                   findAll.where=DbUtilityRadio3000.getFilters(_mainTableObject.filters);
                if(_mainTableObject.fieldsArray)
                    findAll.attributes=_mainTableObject.fieldsArray;

                let include=[];
                let model=DbUtilityRadio3000.getModelNameFormTableName(_mainTableObject.tableName);
                _joinTablesArray.forEach(function(_item)
                {
                    let join={};
                    if(!_item.tableName)
                        throw new Error('joinQuery : _item.tableName is not defined');
                    join.model=modelsSir5[DbUtilityRadio3000.getModelNameFormTableName(_item.tableName)];
                    if(_item.required===false)
                        join.required=_item.required;
                    else
                        join.required=true;
                    if(_item.filters)
                        join.where=DbUtilityRadio3000.getFilters(_item.filters);
                    ...
                    if(_item.joinObject)
                    {
                        ...
                        let joinObject={};
                        joinObject.model=modelsSir5[DbUtilityRadio3000.getModelNameFormTableName(itemJoinObject.tableName)];
                        ...
                        if(itemJoinObject.filters)
                            joinObject.where=DbUtilityRadio3000.getFilters(itemJoinObject.filters);
        ...
        return new Promise(
            function(resolve, reject) {
                ...
                _records.forEach(
                    function(_record)
                    {
                        _record=DbUtilityRadio3000.cleanRecord(_record,model);
                    }
                );
                if(!_transacVar)
    ...
    insertRecord: function(_record,_table){
        ...
        var promise=new Promise(
            function(resolve, reject) {
                var model=DbUtilityRadio3000.getModelNameFormTableName(_table);
                _record=DbUtilityRadio3000.cleanRecord(_record,model);
    ...
    replaceRecord:function(data,table,idToReplaceName,idToReplaceValue)
    {
        ...
        var promise=new Promise(
            function(resolve, reject) {
                DbUtilityRadio3000.deleteRecordById(table,idToReplaceName,idToReplaceValue)
                    .then (
                        function(idToReplaceName)
                        {
                            DbUtilityRadio3000.insertRecord(data,table)
        ...
        var promise=new Promise(
            function(resolve, reject) {
                ...
                async.forEachSeries(dataToBeUpdated, function(record, callback) {
                        DbUtilityRadio3000.updateRecord(record,table,idName,record[idName],transacVar).then(
            ...
            function(resolve, reject) {
                var transacVar=_transacVar||null;
                    async.forEachSeries(recordsToDelete, function(recordToDelete, callback) {
                        DbUtilityRadio3000.deleteRecordById(table,recordIdName,recordToDelete[recordIdName],transacVar).then(
    ...
    getAssociatedAndAvailable:function(associationIdValue,associationIdName,associationTable,availablesTable,nameField,availableIdName,parentTable,parentIdName,parentNameField)
    {
        ...
        return new Promise(
            function(resolve, reject) {
                ...
                var availableModel=DbUtilityRadio3000.getModelNameFormTableName(availablesTable);
                ...
                if(parentTable)
                {
                    p1=DbUtilityRadio3000.read(params,parentTable);
                    promiseArray.push(p1);
                }
                else
                    parentIdValue=1;
                ...
                var p2=DbUtilityRadio3000.joinQuery(mainTableObject,joinTablesArray,"no");
                promiseArray.push(p2);

                var p3=DbUtilityRadio3000.read({limit:'no'},availablesTable);
                promiseArray.push(p3);
...
module.exports = DbUtilityRadio3000;
