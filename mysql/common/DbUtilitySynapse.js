...
let modelsEris  = require('../modelsSynapse');//uncomment for migration
"use strict";

let dbUtilitySynapse = {
    ...
    saveData:function(dataToBeAdded,dataToBeUpdated,dataToBeDeleted,table,idName,_transacVar)
    {
       // console.error(dataToBeUpdated);
        ...
        if(dataToBeAdded.length>0){
            let p1=dbUtilitySynapse.insertRecords(dataToBeAdded,table,transacVar);
            promiseArray.push(p1);
        }
        if(dataToBeUpdated.length>0){
            let p2=dbUtilitySynapse.updateRecords(dataToBeUpdated,table,idName,transacVar);
            promiseArray.push(p2);
        }
        if(dataToBeDeleted.length>0){
            let p3=dbUtilitySynapse.deleteRecordsByIds(dataToBeDeleted,table,idName,transacVar);
    ...
    read: function(_params,_table,_dataBaseName="",_transac){
       ...
       return new Promise(
            function(resolve) {
                ...
                let model=dbUtilitySynapse.getModelNameFormTableName(_table);
                let where=dbUtilitySynapse.getFilters(_params.filters);
                let findAll={};
                findAll.where=where;
                ...
                findAll.raw=true;
                modelsEris[model+dataBaseName].findAll(findAll).then(function(results) {
                    resolve(results);
                });
             });
        ...
        return new Promise(
            function(resolve, reject) {
               ...
               if(_mainTableObject.filters)
                   findAll.where=dbUtilitySynapse.getFilters(_mainTableObject.filters);
                if(_mainTableObject.fieldsArray)
                    findAll.attributes=_mainTableObject.fieldsArray;

                let include=[];
                let model=dbUtilitySynapse.getModelNameFormTableName(_mainTableObject.tableName);
                _joinTablesArray.forEach(function(_item)
                {
                    let join={};
                    if(!_item.tableName)
                        throw new Error('joinQuery : _item.tableName is not defined');
                    join.model=modelsEris[dbUtilitySynapse.getModelNameFormTableName(_item.tableName)];
                    if(_item.required===false)
                        join.required=_item.required;
                    else
                        join.required=true;
                    if(_item.filters)
                        join.where=dbUtilitySynapse.getFilters(_item.filters);
                    ...
                    if(_item.joinObject)
                    {
                        ...
                        let joinObject={};
                        joinObject.model=modelsEris[dbUtilitySynapse.getModelNameFormTableName(itemJoinObject.tableName)];
                        ...
                        if(itemJoinObject.filters)
                            joinObject.where=dbUtilitySynapse.getFilters(itemJoinObject.filters);
                ...
                modelsEris[model+_dataBaseName].findAll(findAll).then(function(results) {
                    resolve(results);
                })
                    .catch(_err=>{
                        console.error(_err);
        ...
        return new Promise(
            function(resolve, reject) {
                ...
                _records.forEach(
                    function(_record)
                    {
                        _record=dbUtilitySynapse.cleanRecord(_record,model);
                    }
                );
                if(!_transacVar)
                    modelsEris[model+databaseName].bulkCreate(_records).then(function() {
                        resolve("true");
                    });
                else {
                    //execute query with transaction
                    modelsEris[model+databaseName].bulkCreate(_records,{transaction:_transacVar}).then(function() {
                        resolve("true");
                    });
                }
             });
    ...
    insertRecord: function(_record,_table){
        ...
        var promise=new Promise(
            function(resolve, reject) {
                var model=dbUtilitySynapse.getModelNameFormTableName(_table);
                _record=dbUtilitySynapse.cleanRecord(_record,model);
                modelsEris[model].create(_record).then(function(_created) {
                    resolve(_created);
                });
            });
        return promise;
    ...
    updateRecord: function(_record,_table,_idName,_idValue,_transacVar){
        ...
        var promise=new Promise(
            function(resolve, reject) {
                ...
                if(!_transacVar)
                {
                    modelsEris[model].update(_record,{where:where}).then(function() {
                        resolve(_record);
                    });
                }
                else
                {
                    modelsEris[model].update(_record,{where:where,transaction:_transacVar}).then(function() {
                        resolve(_record);
                    });
                }
    ...
    updateField: function(_table,_obj,_where,_transacVar){
        ...
        let promise=new Promise(
            function(resolve, reject) {
                ...
                if(!_transacVar)
                {
                    modelsEris[model].update(_obj,{where:_where}).then(function() {
                        resolve(_obj);
                    });
                }
                else
                {
                    modelsEris[model].update(_obj,{where:_where,transaction:_transacVar}).then(function() {
                        resolve(_obj);
                    });
                }
            });
                            ...
                            else
                            {
                                ...
                                if(!_transacVar)
                                {
                                    if(where)
                                    {
                                        ...
                                        modelsEris[model].upsert(_record).then(function(_created) {
                                            resolve(_created);
                                        });
                                    }
                                }
                                else
                                {
                                    ...
                                    {
                                        modelsEris[model].upsert(_record,{transaction:_transacVar}).then(function(_created) {
                                            resolve(_created);
                                        });
                                    }
    ...
    replaceRecord:function(data,table,idToReplaceName,idToReplaceValue)
    {
        ...
        var promise=new Promise(
            function(resolve, reject) {
                dbUtilitySynapse.deleteRecordById(table,idToReplaceName,idToReplaceValue)
                    .then (
                        function(idToReplaceName)
                        {
                            dbUtilitySynapse.insertRecord(data,table)
        ...
        var promise=new Promise(
            function(resolve, reject) {
                ...
                async.forEachSeries(dataToBeUpdated, function(record, callback) {
                        dbUtilitySynapse.updateRecord(record,table,idName,record[idName],transacVar).then(
    ...
    deleteRecordById: function(_table,_recordIdName,_recordIdValue,_transacVar){
        ...
        var promise=new Promise(
            function(resolve, reject) {
                ...
                if(!_transacVar){
                    modelsEris[model].destroy({where:where}).then(function() {
                        resolve(_recordIdValue);
                    });
                }
                else{
                    modelsEris[model].destroy({where:where,transaction: _transacVar}).then(function() {
                        resolve(_recordIdValue);
                    });
                }
            ...
            function(resolve, reject) {
                var transacVar=_transacVar||null;
                    async.forEachSeries(recordsToDelete, function(recordToDelete, callback) {
                        dbUtilitySynapse.deleteRecordById(table,recordIdName,recordToDelete[recordIdName],transacVar).then(
    ...
    getAssociatedAndAvailable:function(associationIdValue,associationIdName,associationTable,availablesTable,nameField,availableIdName,parentTable,parentIdName,parentNameField)
    {
        ...
        return new Promise(
            function(resolve, reject) {
                ...
                var availableModel=dbUtilitySynapse.getModelNameFormTableName(availablesTable);
                ...
                if(parentTable)
                {
                    p1=dbUtilitySynapse.read(params,parentTable);
                    promiseArray.push(p1);
                }
                else
                    parentIdValue=1;
                ...
                var p2=dbUtilitySynapse.joinQuery(mainTableObject,joinTablesArray,"no");
                promiseArray.push(p2);

                var p3=dbUtilitySynapse.read({limit:'no'},availablesTable);
                promiseArray.push(p3);
...
module.exports = dbUtilitySynapse;
