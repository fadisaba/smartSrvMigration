...
let modelsEris  = require('../modelsEris');//uncomment for migration
"use strict";
...
let DbUtilityEris = {
    ...
    saveData:function(dataToBeAdded,dataToBeUpdated,dataToBeDeleted,table,idName,_transacVar)
    {
       // console.error(dataToBeUpdated);
        ...
        if(dataToBeAdded.length>0){
            let p1=DbUtilityEris.insertRecords(dataToBeAdded,table,transacVar);
            promiseArray.push(p1);
        }
        if(dataToBeUpdated.length>0){
            let p2=DbUtilityEris.updateRecords(dataToBeUpdated,table,idName,transacVar);
            promiseArray.push(p2);
        }
        if(dataToBeDeleted.length>0){
            let p3=DbUtilityEris.deleteRecordsByIds(dataToBeDeleted,table,idName,transacVar);
    ...
    read: function(_params,_table,_dataBaseName="",_transac){
       ...
       return new Promise(
            function(resolve) {
                ...
                let model=DbUtilityEris.getModelNameFormTableName(_table);
                let where=DbUtilityEris.getFilters(_params.filters);
                let findAll={};
                findAll.where=where;
        ...
        return new Promise(
            function(resolve, reject) {
               ...
               if(_mainTableObject.filters)
                   findAll.where=DbUtilityEris.getFilters(_mainTableObject.filters);
                if(_mainTableObject.fieldsArray)
                    findAll.attributes=_mainTableObject.fieldsArray;

                let include=[];
                let model=DbUtilityEris.getModelNameFormTableName(_mainTableObject.tableName);
                _joinTablesArray.forEach(function(_item)
                {
                    let join={};
                    if(!_item.tableName)
                        throw new Error('joinQuery : _item.tableName is not defined');
                    join.model=modelsEris[DbUtilityEris.getModelNameFormTableName(_item.tableName)];
                    if(_item.required===false)
                        join.required=_item.required;
                    else
                        join.required=true;
                    if(_item.filters)
                        join.where=DbUtilityEris.getFilters(_item.filters);
                    ...
                    if(_item.joinObject)
                    {
                        ...
                        let joinObject={};
                        joinObject.model=modelsEris[DbUtilityEris.getModelNameFormTableName(itemJoinObject.tableName)];
                        ...
                        if(itemJoinObject.filters)
                            joinObject.where=DbUtilityEris.getFilters(itemJoinObject.filters);
        ...
        return new Promise(
            function(resolve, reject) {
                ...
                _records.forEach(
                    function(_record)
                    {
                        _record=DbUtilityEris.cleanRecord(_record,model);
                    }
                );
                if(!_transacVar)
    ...
    insertRecord: function(_record,_table){
        ...
        var promise=new Promise(
            function(resolve, reject) {
                var model=DbUtilityEris.getModelNameFormTableName(_table);
                _record=DbUtilityEris.cleanRecord(_record,model);
    ...
    replaceRecord:function(data,table,idToReplaceName,idToReplaceValue)
    {
        ...
        var promise=new Promise(
            function(resolve, reject) {
                DbUtilityEris.deleteRecordById(table,idToReplaceName,idToReplaceValue)
                    .then (
                        function(idToReplaceName)
                        {
                            DbUtilityEris.insertRecord(data,table)
        ...
        var promise=new Promise(
            function(resolve, reject) {
                ...
                async.forEachSeries(dataToBeUpdated, function(record, callback) {
                        DbUtilityEris.updateRecord(record,table,idName,record[idName],transacVar).then(
            ...
            function(resolve, reject) {
                var transacVar=_transacVar||null;
                    async.forEachSeries(recordsToDelete, function(recordToDelete, callback) {
                        DbUtilityEris.deleteRecordById(table,recordIdName,recordToDelete[recordIdName],transacVar).then(
    ...
    getAssociatedAndAvailable:function(associationIdValue,associationIdName,associationTable,availablesTable,nameField,availableIdName,parentTable,parentIdName,parentNameField)
    {
        ...
        return new Promise(
            function(resolve, reject) {
                ...
                var availableModel=DbUtilityEris.getModelNameFormTableName(availablesTable);
                ...
                if(parentTable)
                {
                    p1=DbUtilityEris.read(params,parentTable);
                    promiseArray.push(p1);
                }
                else
                    parentIdValue=1;
                ...
                var p2=DbUtilityEris.joinQuery(mainTableObject,joinTablesArray,"no");
                promiseArray.push(p2);

                var p3=DbUtilityEris.read({limit:'no'},availablesTable);
                promiseArray.push(p3);
