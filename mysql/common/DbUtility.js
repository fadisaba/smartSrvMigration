...
let DbUtility = {
    ...
    saveData:function(dataToBeAdded,dataToBeUpdated,dataToBeDeleted,table,idName,_transacVar)
    {
       // console.error(dataToBeUpdated);
        ...
        if(dataToBeAdded.length>0){
            let p1=DbUtility.insertRecords(dataToBeAdded,table,transacVar);
            promiseArray.push(p1);
        }
        if(dataToBeUpdated.length>0){
            let p2=DbUtility.updateRecords(dataToBeUpdated,table,idName,transacVar);
            promiseArray.push(p2);
        }
        if(dataToBeDeleted.length>0){
            let p3=DbUtility.deleteRecordsByIds(dataToBeDeleted,table,idName,transacVar);
                ...
                if (filter.compare)
                {
                    switch (filter.compare)
                    {
                        ...
                        case 'contains':
                            ...
                            break;
                        case 'notIn':
                            where[filter.name]={$notIn:filter.value};
                            break;
                        case 'In':
                            where[filter.name]={$in:filter.value};
                            break;
                        case 'notnull':
                            where[filter.name]={$ne:null};
                            break;
    ...
    execJsonQuery:function(_queryObj,_table,_dataBaseName)
    {
        let model=DbUtility.getModelNameFormTableName(_table);
        return models[model].findAll(_queryObj);
    },
    /**
     *
     * @param _params
    ...
    read: function(_params,_table,_dataBaseName="",_transac){
       ...
       return new Promise(
            function(resolve) {
                ...
                let model=DbUtility.getModelNameFormTableName(_table);
                let where=DbUtility.getFilters(_params.filters);
                let findAll={};
                findAll.where=where;
                ...
                findAll.raw=true;
                models[model+dataBaseName].findAll(findAll).then(function(results) {
                    resolve(results);
                });
             });
    ...
    /**
     * @param _mainTableObject : main table, format {tableName,filters,fieldsArray}
     * @param _joinTablesArray : tables to join, each object in this array has the format {tableName,required,filters,fieldsArray}
     * @param _limit
     * @param _rawData
     * @param _dataBaseName
     * @returns {Promise}
     */
    joinQuery:function(_mainTableObject,_joinTablesArray,_limit,_rawData,_dataBaseName="")
    {
        return new Promise(
            function(resolve, reject) {
               ...
               if(_mainTableObject.filters)
                   findAll.where=DbUtility.getFilters(_mainTableObject.filters);
                if(_mainTableObject.fieldsArray)
                    findAll.attributes=_mainTableObject.fieldsArray;

                let include=[];
                let model=DbUtility.getModelNameFormTableName(_mainTableObject.tableName);
                _joinTablesArray.forEach(function(_item)
                {
                    let join={};
                    if(!_item.tableName)
                        throw new Error('joinQuery : _item.tableName is not defined');
                    join.model=models[DbUtility.getModelNameFormTableName(_item.tableName)];
                    if(_item.required===false)
                        join.required=_item.required;
                    else
                        join.required=true;
                    if(_item.filters)
                        join.where=DbUtility.getFilters(_item.filters);
                    ...
                    if(_item.joinObject)
                    {
                        ...
                        let joinObject={};
                        joinObject.model=models[DbUtility.getModelNameFormTableName(itemJoinObject.tableName)];
                        ...
                        if(itemJoinObject.filters)
                            joinObject.where=DbUtility.getFilters(itemJoinObject.filters);


                        if(_item.joinObject.joinObject){

                            let itemJoinObject=_item.joinObject.joinObject;
                            let joinObject1={};
                            joinObject1.model=models[DbUtility.getModelNameFormTableName(itemJoinObject.tableName)];
                            if(itemJoinObject.required===false)
                                joinObject1.required=itemJoinObject.required;
                            else
                                joinObject1.required=true;

                            if(itemJoinObject.fieldsArray)
                                joinObject1.attributes=itemJoinObject.fieldsArray;

                            if(itemJoinObject.filters)
                                joinObject1.where=DbUtility.getFilters(itemJoinObject.filters);
                            joinObject.include=[joinObject1];
                        }
                        if(_item.joinObject.joinObject2){

                            let itemJoinObject=_item.joinObject.joinObject2;
                            let joinObject1={};
                            joinObject1.model=models[DbUtility.getModelNameFormTableName(itemJoinObject.tableName)];
                            if(itemJoinObject.required===false)
                                joinObject1.required=itemJoinObject.required;
                            ...
                            if(itemJoinObject.filters)
                                joinObject1.where=DbUtility.getFilters(itemJoinObject.filters);
                            joinObject.include.push(joinObject1);
                        }
                ...
                models[model+_dataBaseName].findAll(findAll).then(function(results) {
                    resolve(results);
                });
        ...
        return new Promise(
            function(resolve, reject) {
                ...
                _records.forEach(
                    function(_record)
                    {
                        _record=DbUtility.cleanRecord(_record,model);
                    }
                );
                if(!_transacVar)
                    models[model+databaseName].bulkCreate(_records).then(function() {
                        resolve("true");
                    });
                else {
                    //execute query with transaction
                    models[model+databaseName].bulkCreate(_records,{transaction:_transacVar}).then(function() {
                        resolve("true");
                    });
                }
             });
    ...
    insertRecord: function(_record,_table){
        ...
        var promise=new Promise(
            function(resolve, reject) {
                var model=DbUtility.getModelNameFormTableName(_table);
                _record=DbUtility.cleanRecord(_record,model);
                models[model].create(_record).then(function(_created) {
                    resolve(_created);
                });
            });
        return promise;
    ...
    updateRecord: function(_record,_table,_idName,_idValue,_transacVar){
        ...
        var promise=new Promise(
            function(resolve, reject) {
                ...
                if(!_transacVar)
                {
                    models[model].update(_record,{where:where}).then(function() {
                        resolve(_record);
                    });
                }
                else
                {
                    models[model].update(_record,{where:where,transaction:_transacVar}).then(function() {
                        resolve(_record);
                    });
                }
    ...
    updateField: function(_table,_obj,_where,_transacVar){
        ...
        let promise=new Promise(
            function(resolve, reject) {
                ...
                if(!_transacVar)
                {
                    models[model].update(_obj,{where:_where}).then(function() {
                        resolve(_obj);
                    });
                }
                else
                {
                    models[model].update(_obj,{where:_where,transaction:_transacVar}).then(function() {
                        resolve(_obj);
                    });
                }
            });
        ...
        var promise=new Promise(
                        function(resolve, reject) {
                            ...
                            else
                            {
                                ...
                                if(_record.filtersArray)
                                {

                                    where=DbUtility.getFilters(_record.filtersArray);
                                }
                                _record=DbUtility.cleanRecord(_record,model);
                                //var where={};
                                //  where[_idName]=_idValue;
                                ...
                                if(!_transacVar)
                                {
                                    if(where)
                                    {
                                        ...
                                        models[model].upsert(_record).then(function(_created) {
                                            resolve(_created);
                                        });
                                    }
                                }
                                else
                                {
                                    ...
                                    {
                                        models[model].upsert(_record,{transaction:_transacVar}).then(function(_created) {
                                            resolve(_created);
                                        });
                                    }
        ...
        var promise=new Promise(
            function(resolve, reject) {
                ...
                async.forEachSeries(dataToBeUpdated, function(record, callback) {
                        DbUtility.updateRecord(record,table,idName,record[idName],transacVar).then(
    ...
    deleteRecordById: function(_table,_recordIdName,_recordIdValue,_transacVar){
        ...
        var promise=new Promise(
            function(resolve, reject) {
                ...
                if(!_transacVar){
                    models[model].destroy({where:where}).then(function() {
                        resolve(_recordIdValue);
                    });
                }
                else{
                    models[model].destroy({where:where,transaction: _transacVar}).then(function() {
                        resolve(_recordIdValue);
                    });
                }
            ...
            function(resolve, reject) {
                var transacVar=_transacVar||null;
                    async.forEachSeries(recordsToDelete, function(recordToDelete, callback) {
                        DbUtility.deleteRecordById(table,recordIdName,recordToDelete[recordIdName],transacVar).then(
    ...
    getAssociatedAndAvailable:function(associationIdValue,associationIdName,associationTable,availablesTable,nameField,availableIdName,parentTable,parentIdName,parentNameField,_availableFieldsArray)
    {
        ...
        return new Promise(
            function(resolve, reject) {
                ...
                var availableModel=DbUtility.getModelNameFormTableName(availablesTable);
                ...
                var p2=DbUtility.joinQuery(mainTableObject,joinTablesArray,"no");
                promiseArray.push(p2);

                let p3;
                if(_availableFieldsArray && _availableFieldsArray.length)
                    p3=DbUtility.read({limit:'no',fieldsArray:_availableFieldsArray},availablesTable);
                else
                    p3=DbUtility.read({limit:'no'},availablesTable);
