...
let modelsSmartmed  = require('../modelsSmartmed');//uncomment for migration
"use strict";

let DbUtilitySmartmed = {
    ...
    saveData:function(dataToBeAdded,dataToBeUpdated,dataToBeDeleted,table,idName,_transacVar)
    {
       // console.error(dataToBeUpdated);
        ...
        if(dataToBeAdded.length>0){
            let p1=DbUtilitySmartmed.insertRecords(dataToBeAdded,table,transacVar);
            promiseArray.push(p1);
        }
        if(dataToBeUpdated.length>0){
            let p2=DbUtilitySmartmed.updateRecords(dataToBeUpdated,table,idName,transacVar);
            promiseArray.push(p2);
        }
        if(dataToBeDeleted.length>0){
            let p3=DbUtilitySmartmed.deleteRecordsByIds(dataToBeDeleted,table,idName,transacVar);
    ...
    read: function(_params,_table,_dataBaseName="",_transac){
       ...
       return new Promise(
            function(resolve) {
                ...
                let model=DbUtilitySmartmed.getModelNameFormTableName(_table);
                let where=DbUtilitySmartmed.getFilters(_params.filters);
                let findAll={};
                findAll.where=where;
                ...
                findAll.raw=true;
                modelsSmartmed[model+dataBaseName].findAll(findAll).then(function(results) {
                    resolve(results);
                });
             });
        ...
        return new Promise(
            function(resolve, reject) {
               ...
               if(_mainTableObject.filters)
                   findAll.where=DbUtilitySmartmed.getFilters(_mainTableObject.filters);
                if(_mainTableObject.fieldsArray)
                    findAll.attributes=_mainTableObject.fieldsArray;

                let include=[];
                let model=DbUtilitySmartmed.getModelNameFormTableName(_mainTableObject.tableName);
                _joinTablesArray.forEach(function(_item)
                {
                    let join={};
                    if(!_item.tableName)
                        throw new Error('joinQuery : _item.tableName is not defined');
                    join.model=modelsSmartmed[DbUtilitySmartmed.getModelNameFormTableName(_item.tableName)];
                    if(_item.required===false)
                        join.required=_item.required;
                    else
                        join.required=true;
                    if(_item.filters)
                        join.where=DbUtilitySmartmed.getFilters(_item.filters);
                    ...
                    if(_item.joinObject)
                    {
                        ...
                        let joinObject={};
                        joinObject.model=modelsSmartmed[DbUtilitySmartmed.getModelNameFormTableName(itemJoinObject.tableName)];
                        ...
                        if(itemJoinObject.filters)
                            joinObject.where=DbUtilitySmartmed.getFilters(itemJoinObject.filters);
                ...
                modelsSmartmed[model+_dataBaseName].findAll(findAll).then(function(results) {
                    resolve(results);
                })
                    .catch(_err=>{
                        console.error(_err);
        ...
        return new Promise(
            function(resolve, reject) {
                ...
                _records.forEach(
                    function(_record)
                    {
                        _record=DbUtilitySmartmed.cleanRecord(_record,model);
                    }
                );
                if(!_transacVar)
                    modelsSmartmed[model+databaseName].bulkCreate(_records).then(function() {
                        resolve("true");
                    });
                else {
                    //execute query with transaction
                    modelsSmartmed[model+databaseName].bulkCreate(_records,{transaction:_transacVar}).then(function() {
                        resolve("true");
                    });
                }
             });
    ...
    insertRecord: function(_record,_table){
        ...
        var promise=new Promise(
            function(resolve, reject) {
                var model=DbUtilitySmartmed.getModelNameFormTableName(_table);
                _record=DbUtilitySmartmed.cleanRecord(_record,model);
                modelsSmartmed[model].create(_record).then(function(_created) {
                    resolve(_created);
                });
            });
        return promise;
    ...
    updateRecord: function(_record,_table,_idName,_idValue,_transacVar){
        ...
        var promise=new Promise(
            function(resolve, reject) {
                ...
                if(!_transacVar)
                {
                    modelsSmartmed[model].update(_record,{where:where}).then(function() {
                        resolve(_record);
                    });
                }
                else
                {
                    modelsSmartmed[model].update(_record,{where:where,transaction:_transacVar}).then(function() {
                        resolve(_record);
                    });
                }
    ...
    updateField: function(_table,_obj,_where,_transacVar){
        ...
        let promise=new Promise(
            function(resolve, reject) {
                ...
                if(!_transacVar)
                {
                    modelsSmartmed[model].update(_obj,{where:_where}).then(function() {
                        resolve(_obj);
                    });
                }
                else
                {
                    modelsSmartmed[model].update(_obj,{where:_where,transaction:_transacVar}).then(function() {
                        resolve(_obj);
                    });
                }
            });
    ...
    cleanRecord:function(_record,model)
    {
        for (var key in _record) {
           ...
           /* if(_record[key]===false)
            ...
            if(! key in modelsSmartmed[model])
                delete _record[key];
                            ...
                            else
                            {
                                ...
                                if(!_transacVar)
                                {
                                    if(where)
                                    {
                                        ...
                                        modelsSmartmed[model].upsert(_record).then(function(_created) {
                                            resolve(_created);
                                        });
                                    }
                                }
                                else
                                {
                                    ...
                                    {
                                        modelsSmartmed[model].upsert(_record,{transaction:_transacVar}).then(function(_created) {
                                            resolve(_created);
                                        });
                                    }
    ...
    replaceRecord:function(data,table,idToReplaceName,idToReplaceValue)
    {
        ...
        var promise=new Promise(
            function(resolve, reject) {
                DbUtilitySmartmed.deleteRecordById(table,idToReplaceName,idToReplaceValue)
                    .then (
                        function(idToReplaceName)
                        {
                            DbUtilitySmartmed.insertRecord(data,table)
        ...
        var promise=new Promise(
            function(resolve, reject) {
                ...
                async.forEachSeries(dataToBeUpdated, function(record, callback) {
                        DbUtilitySmartmed.updateRecord(record,table,idName,record[idName],transacVar).then(
    ...
    deleteRecordById: function(_table,_recordIdName,_recordIdValue,_transacVar){
        ...
        var promise=new Promise(
            function(resolve, reject) {
                ...
                if(!_transacVar){
                    modelsSmartmed[model].destroy({where:where}).then(function() {
                        resolve(_recordIdValue);
                    });
                }
                else{
                    modelsSmartmed[model].destroy({where:where,transaction: _transacVar}).then(function() {
                        resolve(_recordIdValue);
                    });
                }
            ...
            function(resolve, reject) {
                var transacVar=_transacVar||null;
                    async.forEachSeries(recordsToDelete, function(recordToDelete, callback) {
                        DbUtilitySmartmed.deleteRecordById(table,recordIdName,recordToDelete[recordIdName],transacVar).then(
    ...
    getAssociatedAndAvailable:function(associationIdValue,associationIdName,associationTable,availablesTable,nameField,availableIdName,parentTable,parentIdName,parentNameField)
    {
        ...
        return new Promise(
            function(resolve, reject) {
                ...
                var availableModel=DbUtilitySmartmed.getModelNameFormTableName(availablesTable);
                ...
                if(parentTable)
                {
                    p1=DbUtilitySmartmed.read(params,parentTable);
                    promiseArray.push(p1);
                }
                else
                    parentIdValue=1;
                ...
                var p2=DbUtilitySmartmed.joinQuery(mainTableObject,joinTablesArray,"no");
                promiseArray.push(p2);

                var p3=DbUtilitySmartmed.read({limit:'no'},availablesTable);
                promiseArray.push(p3);
...
module.exports = DbUtilitySmartmed;
