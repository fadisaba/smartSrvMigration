...
let modelsCabrad  = require('../modelsCabrad');//uncomment for migration
"use strict";
...
let DbUtilityCabrad = {
    ...
    saveData:function(dataToBeAdded,dataToBeUpdated,dataToBeDeleted,table,idName,_transacVar)
    {
       // console.error(dataToBeUpdated);
        ...
        if(dataToBeAdded.length>0){
            let p1=DbUtilityCabrad.insertRecords(dataToBeAdded,table,transacVar);
            promiseArray.push(p1);
        }
        if(dataToBeUpdated.length>0){
            let p2=DbUtilityCabrad.updateRecords(dataToBeUpdated,table,idName,transacVar);
            promiseArray.push(p2);
        }
        if(dataToBeDeleted.length>0){
            let p3=DbUtilityCabrad.deleteRecordsByIds(dataToBeDeleted,table,idName,transacVar);
    ...
    read: function(_params,_table,_dataBaseName="",_transac){
       ...
       return new Promise(
            function(resolve) {
                ...
                let model=DbUtilityCabrad.getModelNameFormTableName(_table);
                let where=DbUtilityCabrad.getFilters(_params.filters);
                let findAll={};
                findAll.where=where;
                ...
                findAll.raw=true;
                modelsCabrad[model+dataBaseName].findAll(findAll).then(function(results) {
                    resolve(results);
                });
             });
        ...
        return new Promise(
            function(resolve, reject) {
               ...
               if(_mainTableObject.filters)
                   findAll.where=DbUtilityCabrad.getFilters(_mainTableObject.filters);
                if(_mainTableObject.fieldsArray)
                    findAll.attributes=_mainTableObject.fieldsArray;

                let include=[];
                let model=DbUtilityCabrad.getModelNameFormTableName(_mainTableObject.tableName);
                _joinTablesArray.forEach(function(_item)
                {
                    let join={};
                    if(!_item.tableName)
                        throw new Error('joinQuery : _item.tableName is not defined');
                    join.model=modelsCabrad[DbUtilityCabrad.getModelNameFormTableName(_item.tableName)];
                    if(_item.required===false)
                        join.required=_item.required;
                    else
                        join.required=true;
                    if(_item.filters)
                        join.where=DbUtilityCabrad.getFilters(_item.filters);
                    ...
                    if(_item.joinObject)
                    {
                        ...
                        let joinObject={};
                        joinObject.model=modelsCabrad[DbUtilityCabrad.getModelNameFormTableName(itemJoinObject.tableName)];
                        ...
                        if(itemJoinObject.filters)
                            joinObject.where=DbUtilityCabrad.getFilters(itemJoinObject.filters);
                ...
                modelsCabrad[model+_dataBaseName].findAll(findAll).then(function(results) {
                    resolve(results);
                })
                    .catch(_err=>{
                        console.error(_err);
        ...
        return new Promise(
            function(resolve, reject) {
                ...
                _records.forEach(
                    function(_record)
                    {
                        _record=DbUtilityCabrad.cleanRecord(_record,model);
                    }
                );
                if(!_transacVar)
                    modelsCabrad[model+databaseName].bulkCreate(_records).then(function() {
                        resolve("true");
                    });
                else {
                    //execute query with transaction
                    modelsCabrad[model+databaseName].bulkCreate(_records,{transaction:_transacVar}).then(function() {
                        resolve("true");
                    });
                }
             });
    ...
    insertRecord: function(_record,_table){
        ...
        var promise=new Promise(
            function(resolve, reject) {
                var model=DbUtilityCabrad.getModelNameFormTableName(_table);
                _record=DbUtilityCabrad.cleanRecord(_record,model);
                modelsCabrad[model].create(_record).then(function(_created) {
                    resolve(_created);
                });
            });
        return promise;
    ...
    updateRecord: function(_record,_table,_idName,_idValue,_transacVar){
        ...
        var promise=new Promise(
            function(resolve, reject) {
                ...
                if(!_transacVar)
                {
                    modelsCabrad[model].update(_record,{where:where}).then(function() {
                        resolve(_record);
                    });
                }
                else
                {
                    modelsCabrad[model].update(_record,{where:where,transaction:_transacVar}).then(function() {
                        resolve(_record);
                    });
                }
    ...
    updateField: function(_table,_obj,_where,_transacVar){
        ...
        let promise=new Promise(
            function(resolve, reject) {
                ...
                if(!_transacVar)
                {
                    modelsCabrad[model].update(_obj,{where:_where}).then(function() {
                        resolve(_obj);
                    });
                }
                else
                {
                    modelsCabrad[model].update(_obj,{where:_where,transaction:_transacVar}).then(function() {
                        resolve(_obj);
                    });
                }
            });
                            ...
                            else
                            {
                                ...
                                if(!_transacVar)
                                {
                                    if(where)
                                    {
                                        ...
                                        modelsCabrad[model].upsert(_record).then(function(_created) {
                                            resolve(_created);
                                        });
                                    }
                                }
                                else
                                {
                                    ...
                                    {
                                        modelsCabrad[model].upsert(_record,{transaction:_transacVar}).then(function(_created) {
                                            resolve(_created);
                                        });
                                    }
    ...
    replaceRecord:function(data,table,idToReplaceName,idToReplaceValue)
    {
        ...
        var promise=new Promise(
            function(resolve, reject) {
                DbUtilityCabrad.deleteRecordById(table,idToReplaceName,idToReplaceValue)
                    .then (
                        function(idToReplaceName)
                        {
                            DbUtilityCabrad.insertRecord(data,table)
        ...
        var promise=new Promise(
            function(resolve, reject) {
                ...
                async.forEachSeries(dataToBeUpdated, function(record, callback) {
                        DbUtilityCabrad.updateRecord(record,table,idName,record[idName],transacVar).then(
    ...
    deleteRecordById: function(_table,_recordIdName,_recordIdValue,_transacVar){
        ...
        var promise=new Promise(
            function(resolve, reject) {
                ...
                if(!_transacVar){
                    modelsCabrad[model].destroy({where:where}).then(function() {
                        resolve(_recordIdValue);
                    });
                }
                else{
                    modelsCabrad[model].destroy({where:where,transaction: _transacVar}).then(function() {
                        resolve(_recordIdValue);
                    });
                }
            ...
            function(resolve, reject) {
                var transacVar=_transacVar||null;
                    async.forEachSeries(recordsToDelete, function(recordToDelete, callback) {
                        DbUtilityCabrad.deleteRecordById(table,recordIdName,recordToDelete[recordIdName],transacVar).then(
    ...
    getAssociatedAndAvailable:function(associationIdValue,associationIdName,associationTable,availablesTable,nameField,availableIdName,parentTable,parentIdName,parentNameField)
    {
        ...
        return new Promise(
            function(resolve, reject) {
                ...
                var availableModel=DbUtilityCabrad.getModelNameFormTableName(availablesTable);
                ...
                if(parentTable)
                {
                    p1=DbUtilityCabrad.read(params,parentTable);
                    promiseArray.push(p1);
                }
                else
                    parentIdValue=1;
                ...
                var p2=DbUtilityCabrad.joinQuery(mainTableObject,joinTablesArray,"no");
                promiseArray.push(p2);

                var p3=DbUtilityCabrad.read({limit:'no'},availablesTable);
                promiseArray.push(p3);
...
module.exports = DbUtilityCabrad;
